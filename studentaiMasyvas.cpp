...
float vidurkis(int pazymiai[], int n)
{
...
float mediana(int pazymiai[], int n)
{
    ...
    if (n % 2 == 0)
    {
        return (float)(pazymiai[n / 2 - 1] + pazymiai[n / 2]) / 2;
    }
    else
    {
        return (float)pazymiai[n / 2];
    }
}
...
void pazymiuIvedimas(Asmuo& stud)
{
    ...
    stud.pazymiai = new int[1];
    int pazymiuSk = 1;
    int dabartinisDydis = 1;
    ...
    while (arIvestas == true)
    {
        ...
        if ((skaicius == true) || (pazymys == "-1"))
        {
            ...
            if (paz >= 0 && paz <= 10)
            {
                if (dabartinisDydis < pazymiuSk)
                {
                    int* temp = new int[pazymiuSk];

                    for (int i = 0; i < pazymiuSk - 1; i++)
                    {
                        temp[i] = stud.pazymiai[i];
                    }

                    delete[] stud.pazymiai;

                    stud.pazymiai = new int[pazymiuSk];
                    for (int i = 0; i < pazymiuSk - 1; i++)
                    {
                        stud.pazymiai[i] = temp[i];
                    }
                    delete[] temp;
                    dabartinisDydis++;
                }

                stud.pazymiai[pazymiuSk - 1] = paz;
                pazymiuSk++;
                arNulis = false;
            }
    ...
    stud.kiekPazymiu = dabartinisDydis;
}
...
void atsitiktiniaiPazymiai(Asmuo& stud)
{
    ...
    if (kiek < 1) kiek = 1;
    stud.pazymiai = new int[kiek];
    stud.kiekPazymiu = kiek;
...
float galutinisPazymys(Asmuo stud, string skaiciavimoBudas)
{
    ...
    {
        semestroRez = vidurkis(stud.pazymiai, stud.kiekPazymiu);
    }
    else if (skaiciavimoBudas == "b") // naudojama mediana
    {
        semestroRez = mediana(stud.pazymiai, stud.kiekPazymiu);
    }
    else
    {
